 ...
 *
 */
char dma_buf[0x100000] __aligned(0x400);
unsigned __net_dma_start = (unsigned)dma_buf;
unsigned __net_dma_end = (unsigned)dma_buf + 0x100000;
...
static int g_mac_mode = 2;//MAC_MODE_RMII_CLK_EXTERNAL;

static int g_debug = 4;
#define ET_DEBUG
/*
M6TV
 23
...
{
	flush_dcache_range(ALIGN_DOWN(startAddr, ARCH_DMA_MINALIGN), ALIGN(endAddr, ARCH_DMA_MINALIGN));
	return;
}

static inline void _dcache_inv_range_for_net(unsigned startAddr, unsigned endAddr)
{
	invalidate_dcache_range(ALIGN_DOWN(startAddr, ARCH_DMA_MINALIGN), ALIGN(endAddr, ARCH_DMA_MINALIGN));
	return;
}
...
#define NET_MAX_RESET_TEST 1000
	...
	for (i = 0; i < NET_MAX_RESET_TEST; i++) {
		...
		printf("writel(%x, %p)\n", ETH_DMA_0_Bus_Mode_SWR, (void *)ETH_DMA_0_Bus_Mode);
...
static int aml_eth_send(struct udevice *net_current, void *packet, int length)
{
	unsigned int mask;
	unsigned int status;
...
static const struct udevice_id aml_eth_ids[] = {
	{ .compatible = "aml,eth" },
	{ },
};

void aml_eth_halt(struct udevice *dev) {
}

static const struct eth_ops aml_eth_ops = {
	.start = aml_ethernet_init,
	.send = aml_eth_send,
	.recv = aml_eth_rx,
	.stop = aml_eth_halt,
};
