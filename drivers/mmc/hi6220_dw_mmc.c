...
static int hi6220_dwmmc_of_to_plat(struct udevice *dev)
{
...
#if CONFIG_ENABLED(CLK) && CONFIG_ENABLED(RESET)
	...
	if (ret) {
		dev_err("Failed to get resets(ret = %d)", ret);
...
static int hi6220_dwmmc_probe(struct udevice *dev)
{
	...
	if (ret) {
		dev_err("Failed to enable ciu clock(ret = %d).\n", ret);
		return log_msg_ret("clk", ret);
	}

	ret = reset_deassert_bulk(&priv->rsts);
	if (ret) {
		dev_err("Failed to deassert resets(ret = %d).\n", ret)
		return log_msg_ret("rst", ret);
	}

	host->bus_hz = clk_get_rate(priv->clks[HI6220_DWMMC_CLK_CIU]);
	if (host->bus_hz <= 0) {
		dev_err("Failed to get ciu clock rate(ret = %d).\n", ret);
		return log_msg_ret("clk", ret);
