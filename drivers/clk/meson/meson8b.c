				     ...
				     struct meson_clk_hw_data *hw_clks)
{
	...
	for (i = CLKID_PLL_FIXED; i < hw_clks->num; i++) {
		...
		if (!clk) {
			log_err("Error %ld registering clock <%s>\n", PTR_ERR(clk), hw_clks->hws[i]->name);
			continue;
		}
